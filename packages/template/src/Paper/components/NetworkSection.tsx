import type { NetfaceInfo } from '@/types'
        ...
        {networks.map((net, i) => (
          <article key={i} className="network-card">
            ...
            <div className="network-metrics">
              <MetricCard label="上传" speed={formatBytes(net.upSpeed, { from: 'KB' }) + '/s'} total={formatBytes(net.totalUp, { from: 'MB' })} type="up" />
              <MetricCard label="下载" speed={formatBytes(net.downSpeed, { from: 'KB' }) + '/s'} total={formatBytes(net.totalDown, { from: 'MB' })} type="down" />
            </div>
          </article>
        ))}
      </div>
